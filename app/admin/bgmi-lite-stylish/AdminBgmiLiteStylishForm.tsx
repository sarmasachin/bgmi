...
import type { BgmiLiteStylishNamePageContent } from "@/src/lib/bgmiLiteStylishNamePage";
...
export type StylishAdminSectionId = "seo" | "copy" | "article" | "faq";

type Props = {
  page: BgmiLiteStylishNamePageContent;
  openIds: Set<StylishAdminSectionId>;
  onToggle: (id: StylishAdminSectionId) => void;
  onPatch: (
...
export function AdminBgmiLiteStylishForm({ page, openIds, onToggle, onPatch }: Props) {
  return (
    <>
      <Section id="seo" title="SEO" open={openIds.has("seo")} onToggle={onToggle}>
        <p style={{ margin: "0 0 10px", fontSize: 12, color: "#94a3b8" }}>
          URL path stays locked to <code>/bgmi-lite-stylish-name</code>.
        </p>
        <Field
          label="SEO title"
          value={page.seoTitle}
      ...
      <Section id="article" title="Article" open={openIds.has("article")} onToggle={onToggle}>
        ...
        <div className="admin-pages-editor-wrap" style={{ marginBottom: 12 }}>
          <RichTextEditor
            ...
            storageKey="bgmi_admin_lite_stylish_article_v1"
          />
        </div>
        <Field
          label="Comments lead"
