...
export default function AdminSettingsPage() {
  ...
  const [phoneModelsText, setPhoneModelsText] = useState("");
  const [phoneModelsMessage, setPhoneModelsMessage] = useState("");
  const [headerSiteTitle, setHeaderSiteTitle] = useState("SENS MASTER PRO");
  const [homeHeroTitle, setHomeHeroTitle] = useState("SENS MASTER PRO");
  const [titleTemplate, setTitleTemplate] = useState("%s | SENS MASTER PRO");
  ...
  const [showShareRail, setShowShareRail] = useState(true);
  const [footerCopyright, setFooterCopyright] = useState(
    "© 2026 SENS MASTER PRO. All rights reserved.",
  );
  const [footerBrandTitle, setFooterBrandTitle] = useState("SENS MASTER PRO");
  const [footerTagline, setFooterTagline] = useState(
    "BGMI tools, news updates, and pro-level sensitivity insights.",
  );
  ...
  return (
    <section className="admin-section">
      ...
      {!settingsReady ? (
        ...
        <div className="form-group">
          <label className="admin-ads-enable" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            />
            <span>Show social share button to visitors (floating rail on pages)</span>
          </label>
        </div>
        <div className="form-group">
      ...
      <p style={{ marginBottom: 12, opacity: 0.9 }}>
        Questions and answers under “अक्सर पूछे जाने वाले प्रश्न (FAQ)” on the home page. Both fields
