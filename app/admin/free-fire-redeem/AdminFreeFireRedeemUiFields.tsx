...
import type { FreeFireRedeemUiLabels } from "@/src/lib/freeFireRedeemUiDefaults";

type Props = {
  ui: FreeFireRedeemUiLabels;
  onPatchUi: (patch: Partial<FreeFireRedeemUiLabels>) => void;
};

const FIELDS: Array<{ key: keyof FreeFireRedeemUiLabels; label: string; multiline?: boolean }> = [
...
export function AdminFreeFireRedeemUiFields({ ui, onPatchUi }: Props) {
  return (
    <>
      <p style={{ margin: "0 0 12px", fontSize: 12, color: "#94a3b8", lineHeight: 1.45 }}>
        URL path stays locked to <code>/free-fire-redeem-code</code>. Comment approve/delete stays in{" "}
...
export function patchRedeemPageUi(
  prev: FreeFireRedeemCodePageContent,
  patch: Partial<FreeFireRedeemUiLabels>,
