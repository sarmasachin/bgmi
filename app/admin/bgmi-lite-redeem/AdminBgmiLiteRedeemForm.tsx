...
import type { BgmiLiteRedeemCodeItem, BgmiLiteRedeemCodePageContent } from "@/src/lib/bgmiLiteRedeemCodes";
import {
  AdminBgmiLiteRedeemUiFields,
  patchRedeemPageUi,
} from "./AdminBgmiLiteRedeemUiFields";
...
type Props = {
  ...
  onPatch: (updater: (prev: BgmiLiteRedeemCodePageContent) => BgmiLiteRedeemCodePageContent) => void;
};
...
function emptyCode(): BgmiLiteRedeemCodeItem {
  return {
    id: `new-${Date.now()}`,
...
export function AdminBgmiLiteRedeemForm({ page, openIds, onToggle, onPatch }: Props) {
  function patchCode(index: number, patch: Partial<BgmiLiteRedeemCodeItem>) {
    onPatch((prev) => {
    ...
    <>
      ...
      <Section id="article" title="Article (below codes)" open={openIds.has("article")} onToggle={onToggle}>
        <p style={{ margin: "0 0 10px", fontSize: 12, color: "#94a3b8", lineHeight: 1.45 }}>
          Insert image se har image pe <strong>alt text</strong> prompt aayega. Multiple images = har
          baar alag alt likho. Empty chhoda to fallback <code>article-image</code> lagta hai — SEO ke
          liye descriptive alt better hai.
        </p>
        <div className="admin-pages-editor-wrap" style={{ marginBottom: 12 }}>
          <RichTextEditor
            ...
            storageKey="bgmi_admin_lite_redeem_article_v1"
          />
        </div>
        <Field
