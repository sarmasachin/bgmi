...
import type { PubgMobileLiteNamePageContent } from "@/src/lib/pubgMobileLiteNamePage";
...
export type NameAdminSectionId = "seo" | "copy" | "article" | "faq";
...
export function AdminPubgMobileLiteNameForm({ page, openIds, onToggle, onPatch }: Props) {
  return (
    <>
      <Section id="seo" title="SEO" open={openIds.has("seo")} onToggle={onToggle}>
        <p style={{ margin: "0 0 10px", fontSize: 12, color: "#94a3b8" }}>
          URL path stays locked to <code>/pubg-mobile-lite-name</code>.
        </p>
        <Field
          label="SEO title"
          value={page.seoTitle}
      ...
      <Section id="article" title="Article" open={openIds.has("article")} onToggle={onToggle}>
        ...
        <div className="admin-pages-editor-wrap" style={{ marginBottom: 12 }}>
          <RichTextEditor
            ...
            storageKey="bgmi_admin_pml_name_article_v1"
          />
        </div>
        <Field
          label="Comments lead"
