import {
  ...
  DEFAULT_FREE_FIRE_STYLISH_NAME_PAGE,
...
export const FREE_FIRE_STYLISH_NAME_SETTING_KEY = "settings:freeFireStylishName";
...
export function normalizeFreeFireStylishNamePage(raw: unknown): FreeFireStylishNamePageContent {
  const defaults = DEFAULT_FREE_FIRE_STYLISH_NAME_PAGE;
  if (!isPlainObject(raw)) return cloneFreeFireStylishNamePage(defaults);
...
export async function getFreeFireStylishNamePage(): Promise<FreeFireStylishNamePageContent> {
  const row = await tryPrisma(async () =>
    prisma.siteSetting.findUnique({ where: { key: FREE_FIRE_STYLISH_NAME_SETTING_KEY } }),
  );
  if (row === null || !row?.value) return cloneFreeFireStylishNamePage();
  return normalizeFreeFireStylishNamePage(row.value);
}

export async function getFreeFireStylishNamePageForAdmin(): Promise<{
  ...
  if (row === null || !row?.value) {
    return { page: cloneFreeFireStylishNamePage(), usingDefault: true };
  }
  return { page: normalizeFreeFireStylishNamePage(row.value), usingDefault: false };
}

export async function saveFreeFireStylishNamePage(
  raw: unknown,
): Promise<{ page: FreeFireStylishNamePageContent; usingDefault: boolean }> {
  const page = normalizeFreeFireStylishNamePage(raw);
  const saved = await tryPrismaLong(async () => {
    await prisma.siteSetting.upsert({
      where: { key: FREE_FIRE_STYLISH_NAME_SETTING_KEY },
      create: { key: FREE_FIRE_STYLISH_NAME_SETTING_KEY, value: page },
      update: { value: page },
    });
    return true;
  });
...
export async function clearFreeFireStylishNamePage(): Promise<{
  page: FreeFireStylishNamePageContent;
  usingDefault: boolean;
}> {
  const deleted = await tryPrismaLong(async () => {
    await prisma.siteSetting.deleteMany({ where: { key: FREE_FIRE_STYLISH_NAME_SETTING_KEY } });
    return true;
  });
