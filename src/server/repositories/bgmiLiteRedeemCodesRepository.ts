import {
  cloneBgmiLiteRedeemPage,
  DEFAULT_BGMI_LITE_REDEEM_PAGE,
  type BgmiLiteRedeemCodeItem,
  type BgmiLiteRedeemCodePageContent,
  type BgmiLiteRedeemFaqItem,
...
function normalizeCode(raw: unknown, index: number): BgmiLiteRedeemCodeItem | null {
  ...
  const item: BgmiLiteRedeemCodeItem = { id, title, code, status };
...
function normalizeFaq(raw: unknown, index: number): BgmiLiteRedeemFaqItem | null {
  if (!isPlainObject(raw)) return null;
...
function normalizeUi(raw: unknown): BgmiLiteRedeemUiLabels {
  const defaults = DEFAULT_BGMI_LITE_REDEEM_UI;
  const src = isPlainObject(raw) ? raw : {};
  const pick = (key: keyof BgmiLiteRedeemUiLabels) =>
...
export function normalizeBgmiLiteRedeemPage(raw: unknown): BgmiLiteRedeemCodePageContent {
...
export async function getBgmiLiteRedeemPage(): Promise<BgmiLiteRedeemCodePageContent> {
  ...
  return normalizeBgmiLiteRedeemPage(row.value);
}

export async function getBgmiLiteRedeemUpdatedAt(): Promise<Date | null> {
...
export async function getBgmiLiteRedeemPageForAdmin(): Promise<{
  ...
  return { page: normalizeBgmiLiteRedeemPage(row.value), usingDefault: false };
}

export async function saveBgmiLiteRedeemPage(
  raw: unknown,
): Promise<{ page: BgmiLiteRedeemCodePageContent; usingDefault: boolean }> {
  const page = normalizeBgmiLiteRedeemPage(raw);
...
export async function clearBgmiLiteRedeemPage(): Promise<{
  page: BgmiLiteRedeemCodePageContent;
  usingDefault: boolean;
}> {
