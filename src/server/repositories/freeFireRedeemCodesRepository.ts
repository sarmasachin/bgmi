...
function normalizeUi(raw: unknown): FreeFireRedeemUiLabels {
  const defaults = DEFAULT_FREE_FIRE_REDEEM_UI;
...
export function normalizeFreeFireRedeemPage(raw: unknown): FreeFireRedeemCodePageContent {
...
export async function getFreeFireRedeemPage(): Promise<FreeFireRedeemCodePageContent> {
  ...
  return normalizeFreeFireRedeemPage(row.value);
}

export async function getFreeFireRedeemUpdatedAt(): Promise<Date | null> {
  const row = await tryPrismaLong(async () =>
...
export async function getFreeFireRedeemPageForAdmin(): Promise<{
  ...
  return { page: normalizeFreeFireRedeemPage(row.value), usingDefault: false };
}

export async function saveFreeFireRedeemPage(
  raw: unknown,
): Promise<{ page: FreeFireRedeemCodePageContent; usingDefault: boolean }> {
  const page = normalizeFreeFireRedeemPage(raw);
...
export async function clearFreeFireRedeemPage(): Promise<{
  page: FreeFireRedeemCodePageContent;
  usingDefault: boolean;
}> {
